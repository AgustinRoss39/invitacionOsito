...
function fitNameTitles() {
  fitTextToContainer($("#loader-title"), 60, 28);
  fitTextToContainer($("#guest-name"), 70, 30);
}
...
function populateInvitation() {
  ...
  setText("#confirmation-note", invitation.confirmationNote);
...
function showCharacter(index) {
  ...
  window.setTimeout(() => {
    ...
    characterImage.style.transform = `translate3d(${character.translateX}, 18px, 0) scale(1.02)`;
...
function updateCountdown() {
  ...
  if (now >= end) {
    finishCountdown("¡Gracias por acompañarme en este vuelo! ✈️");
    return;
  }

  if (now >= start) {
    finishCountdown("¡Hoy despegamos! ☁️", "¡La aventura ya empezó!");
    return;
  }
  ...
  if (days === 0) {
    ...
    countdownStatus.textContent = `Faltan ${days} días para el despegue.`;
  }
}
